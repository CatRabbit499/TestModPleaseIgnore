...
public class CommandGamma{
	...
	public static void register(CommandDispatcher<CommandSource> dispatcher){
		dispatcher.register(literal("gamma")
			.then(argument("newGamma",FloatArgumentType.floatArg(0,1000)).executes(ctx->setGamma(ctx.getSource(),
				FloatArgumentType.getFloat(ctx,"newGamma"))))
	...
	private static int setGamma(CommandSource source,float amount){
		...
		ChatUtils.respond(source,true,Template.valid("Gamma Updated: "),Minecraft.getInstance().gameSettings.gammaSetting);
		return 1;
	}
	
	private static int getGamma(CommandSource source){
		ChatUtils.respond(source,true,Template.valid("Gamma: "),Minecraft.getInstance().gameSettings.gammaSetting);
		return 1;
	}
}